...
def extract_pdf(pdf_path):
    ...
    if not result['bs_asset'] and not result['pl']:
        ...
        print(f'         試算表や勘定科目残高一覧など非標準形式の可能性があります。')
    return result
...
def write_sga(ws, all_data, periods):
    write_title(ws, 1, '販売費及び一般管理費　（単位：千円）')
    write_notice(ws, 2)
    write_hdr(ws, 3, periods)
...
def write_valuation(ws, all_data, comp_df, medians, industry):
    ...
    rng = f'{net_assets*2:,.0f} 〜 {net_assets*4:,.0f} 千円' if (net_assets and net_assets>0) else \
    ...
    sec('◆ マルチプル法　算定結果　（単位：千円）')
    for col, h in enumerate(['指標','中央値マルチプル','使用財務数値（千円）','算定企業価値（千円）','算定根拠',''], 1):
...
def main():
    ...
    if not pdf_paths:
        found = sorted(script_dir.glob('*.pdf'), key=lambda p: p.name)
        # 出力済みExcelのバックアップPDFなど除外はせず全件表示
        if not found:
            ...
            input('Enterキーで終了...')
            sys.exit(1)
        print(f'PDFを自動検出しました ({len(found)}件):')
        for i, p in enumerate(found, 1):
            print(f'  {i}. {p.name}')
        if len(found) > 3:
            print(f'  ※ ファイル名順で最初の3件を使用します')
        pdf_paths = [str(p) for p in found[:3]]
        print()
