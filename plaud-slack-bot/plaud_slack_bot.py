...
def fetch_recordings_by_date(token, base_url, yesterday_str):
    recordings = []
    seen_ids = set()
    ...
    for page in range(1, MAX_PAGES + 1):
        ...
        for f in files:
            date_str = to_jst_date_str(f.get("start_time") or f.get("create_time") or f.get("created_at"))
            if not date_str:
                continue
            if date_str >= yesterday_str:
                any_recent_file = True
            if date_str == yesterday_str and f.get("id") not in seen_ids:
